...
class MenuPrice(ModelBase):
    ...
    id_product: Mapped[int] = mapped_column(
        sa.BigInteger, sa.ForeignKey("products.id", ondelete="CASCADE")
    )
    product: Mapped["Product"] = orm.relationship("Product", lazy="joined")

    id_variation: Mapped[int] = mapped_column(
        sa.BigInteger, sa.ForeignKey("variations.id", ondelete="CASCADE")
    )
